...
namespace Zoro_Web_API.Controllers
{
    public class Contract
    {
        ...
        //    foreach (var scripthash in scripthashs)
        //    {
        //        ScriptBuilder tmpSb = new ScriptBuilder();
        //        httpHelper hh = new httpHelper();
        //        var json = MyJson.Parse(JsonConvert.SerializeObject(paramsJA[n])).AsList();
        //        var list = json.AsList();
        //        for (int i = list.Count - 1; i >= 0; i--)
        ...
        //        var scripthashReverse = Helper.HexString2Bytes(scripthash).Reverse().ToArray();
        //        tmpSb.EmitAppCall(scripthashReverse);
        //        string invokeSc = Helper.Bytes2HexString(tmpSb.ToArray());
